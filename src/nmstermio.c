...
static char foregroundColor[8] = COLOR_BLUE;          // Foreground color as hex string
 ...
 * "white", "yellow", "magenta", "blue", "green", "red", "cyan", or
 * a hex color like "#FF5500" or "ff5500".
 */
void nmstermio_set_foregroundcolor(char *c) {
	...
	if (strlen(hex) == 6 && sscanf(hex, "%02x%02x%02x", &r, &g, &b) == 3) {
		foregroundColor[0] = '#';
		memcpy(foregroundColor + 1, hex, 6);
		foregroundColor[7] = '\0';
		return;
	}
